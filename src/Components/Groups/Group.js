...
function Group() {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('http://localhost:3001/api/card/fetchGroup');
        ...
        <div>
            <div className='group-name'>
                <div className='group-events'>
                    <h4 className='add-group-h1'>Add Group</h4>
                </div>
                <div className='group-detail'>
                    <div className='group-gap'>
                    {tagDetails.map((group) => (
                            <button className='add-events-butn' key={group.id}>
                                <span className='groupbtn'>{group.groupname}</span>
                            </button>
                        ))}
                   
                        <button className='add-group-butnss'>
                ...
                <div className='btnnss'>
                    <button className='groupsavebtn'><span className='groupssavebtn'> Save</span></button>
                    <button className='groupcancelbtn'><span className='groupsscancelbtn'>Cancel</span> </button>
                </div>
            </div>
